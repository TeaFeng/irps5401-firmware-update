...
int rsasignature(char* data, size_t data_len, char* signature, unsigned int* sig_len) {
	...
	// 获取 SHA-256 算法
	const EVP_MD* md = EVP_MD_fetch(NULL, "SHA2-256", NULL);
	if (md == NULL) {
		fprintf(stderr, "Failed to fetch SHA-256 algorithm.\n");
		handleErrors();
	}
	...
	// 加载私钥
	FILE* key_fp = fopen(PRIVATE_KEY_PATH, "rb");
	if (key_fp == NULL) {
	...
	// 获取签名的最大长度
	size_t siglen;
	if (EVP_PKEY_sign(pctx, NULL, &siglen, hash, hash_len) <= 0) {
	...
	// 分配内存用于存储签名
	unsigned char* p_signature = OPENSSL_malloc(siglen);
	if (!p_signature) {
	...
	// 生成签名
	if (EVP_PKEY_sign(pctx, p_signature, &siglen, hash, hash_len) <= 0) {
	...
	*sig_len = siglen;
	...
	FILE* pub_key_fp = fopen(PUBLIC_KEY_PATH, "rb");
	if (pub_key_fp == NULL) {
	...
	EVP_PKEY* pub_pkey = PEM_read_PUBKEY(pub_key_fp, NULL, NULL, NULL);
	fclose(pub_key_fp);
	if (pub_pkey == NULL) {
	...
	// 创建验证上下文
	EVP_PKEY_CTX* vctx = EVP_PKEY_CTX_new(pub_pkey, NULL);
	if (!vctx) {
		fprintf(stderr, "Failed to create verification PKEY_CTX.\n");
		OPENSSL_free(p_signature);
	...
	// 设置验证算法和参数
	if (EVP_PKEY_CTX_set_signature_md(vctx, md) <= 0) {
		fprintf(stderr, "Failed to set verification message digest.\n");
		OPENSSL_free(p_signature);
		EVP_PKEY_CTX_free(vctx);
		EVP_PKEY_free(pub_pkey);
		EVP_MD_CTX_free(mdctx);
	...
	int result = EVP_PKEY_verify(vctx, p_signature, siglen, hash, hash_len);
	if (result <= 0) {
